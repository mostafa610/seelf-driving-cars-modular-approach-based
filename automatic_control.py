...
def game_loop(args):
    ...
    try:
        ...
        for w in waypoints:
            world.world.debug.draw_string(w.transform.location, 'X', draw_shadow=False,
        ...
        exodian = Exodian(world.player , adjust_speed=False)
        spawn_point = random.choice(world.map.get_spawn_points())
        dest_location  = spawn_point.location
        start_location = world.player.get_location()

        while start_location.distance(dest_location) <= 10: # Less than 10 meters
            spawn_point = random.choice(world.map.get_spawn_points())
            dest_location  = spawn_point.location 

        exodian.set_destination((spawn_point.location.x,
                               spawn_point.location.y,
                               spawn_point.location.z))        
        exodian.recording_camera_attachment()
